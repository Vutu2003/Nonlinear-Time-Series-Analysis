...
# Halsey et al. (1986) và Kantelhardt et al. (2002)
Ý tưởng về tham số $q$ trong MF-DFA có nguồn gốc từ **multifractal formalism** do Halsey et al. (1986) đề xuất. Trong phương pháp box-counting, hàm phân hoạch được định nghĩa dưới dạng $Z(q,\epsilon)=\sum_i \mu_i^q$, trong đó $q$ đóng vai trò điều chỉnh trọng số của các ô (box): với $q>0$, các ô có xác suất lớn được nhấn mạnh, trong khi với $q<0$, các ô có xác suất nhỏ chiếm ưu thế. Kantelhardt et al. (2002) kế thừa ý tưởng này nhưng thay thế đại lượng xác suất $\mu_i$ bằng hàm dao động cục bộ $F(s,\nu)$ thu được từ DFA. Nhờ đó, MF-DFA có thể khảo sát các đặc trưng scaling của chuỗi thời gian không dừng mà không cần xây dựng trực tiếp một độ đo (measure), đồng thời vẫn giữ nguyên vai trò của tham số $q$ như một "tham số quan sát", cho phép nhấn mạnh các vùng dao động lớn ($q>0$) hoặc các vùng dao động nhỏ ($q<0$).

# Giả định của MF-DFA

Mặc dù MF-DFA được thiết kế để phân tích chuỗi thời gian không dừng (nonstationary), phương pháp vẫn dựa trên một số giả định quan trọng:

- **Giả định về tính scaling:** Sau khi loại bỏ xu hướng cục bộ, hàm thăng giáng \(F_q(s)\) được giả định tuân theo quy luật lũy thừa theo kích thước cửa sổ \(s\). Nếu tồn tại nhiều miền scaling hoặc các điểm chuyển tiếp (crossovers), việc ước lượng \(h(q)\) có thể bị sai lệch.

- **Giả định về detrending:** Xu hướng cục bộ của tín hiệu được giả định có thể xấp xỉ bằng đa thức bậc hữu hạn. Với các xu hướng phức tạp hơn, bước detrending có thể không loại bỏ hoàn toàn ảnh hưởng của nonstationarity.

- **Giả định tiệm cận:** MF-DFA yêu cầu chuỗi dữ liệu đủ dài và tồn tại một khoảng scaling đủ rộng để ước lượng chính xác các số mũ scaling. Với dữ liệu hữu hạn, các hiện tượng như **systematic bias**, **finite-size effect** và **spurious multifractality** có thể xuất hiện.

Do đó, kết quả MF-DFA nên được kiểm chứng bằng các thực nghiệm bổ sung như **shuffle test**, mô phỏng Monte Carlo hoặc so sánh với các mô hình chuẩn nhằm phân biệt giữa multifractality thực sự và các sai lệch do giả định hoặc kích thước mẫu hữu hạn.

## Hạn chế của MF-DFA

Mặc dù MF-DFA là một trong những phương pháp phổ biến để phân tích đa phân dạng của chuỗi thời gian không dừng, thuật toán vẫn tồn tại một số hạn chế:

- **Nhạy cảm với việc lựa chọn khoảng scaling:** Việc ước lượng \(h(q)\) phụ thuộc trực tiếp vào miền khớp tuyến tính trên đồ thị log-log. Lựa chọn khoảng scaling không phù hợp có thể dẫn đến sai số hoặc kết luận sai về tính đa phân dạng.

- **Phụ thuộc vào các tham số của thuật toán:** Các tham số như bậc đa thức detrending, tập giá trị \(q\), phạm vi scale và số lượng scale đều ảnh hưởng đến kết quả cuối cùng. Hiện chưa có một quy tắc chung tối ưu cho mọi loại dữ liệu.

- **Độ ổn định giảm ở các giá trị \(|q|\) lớn:** Khi \(|q|\) tăng, kết quả trở nên nhạy cảm với các dao động cực trị và nhiễu, làm tăng phương sai của các ước lượng \(h(q)\), \(\tau(q)\) và phổ kỳ dị \(f(\alpha)\).

- **Không xác định trực tiếp nguồn gốc của multifractality:** MF-DFA chỉ phát hiện sự tồn tại của tính đa phân dạng mà không thể kết luận nguyên nhân. Các phân tích bổ sung như shuffle test hoặc surrogate test thường được sử dụng để phân biệt ảnh hưởng của tương quan dài hạn và phân phối xác suất.

- **Chi phí tính toán tương đối lớn:** Thuật toán phải lặp lại phép detrending trên nhiều scale và nhiều giá trị \(q\), khiến thời gian tính toán tăng đáng kể đối với chuỗi dữ liệu dài hoặc các nghiên cứu Monte Carlo.